...
printf '[deploy] Running remote installer (ENABLE_CROSSFEED=%s INSTALL_AAC_PLUGIN=%s RESTART_GADGET_NOW=%s SKIP_APT=%s)\n' "${ENABLE_CROSSFEED:-interactive}" "${INSTALL_AAC_PLUGIN:-interactive}" "$RESTART_GADGET_NOW" "$SKIP_APT"
remote_dir_q="$(sh_quote "$REMOTE_DIR")"
airpods_mac_q="$(sh_quote "$AIRPODS_MAC")"
eq_q="$(sh_quote "$ENABLE_AIRPODS_PRO3_EQ")"
crossfeed_q="$(sh_quote "$ENABLE_CROSSFEED")"
aac_plugin_q="$(sh_quote "$INSTALL_AAC_PLUGIN")"
...
ssh -t "${SSH_OPTS[@]}" "$PI_HOST" "cd ${remote_dir_q} && AIRPODS_MAC=${airpods_mac_q} ENABLE_AIRPODS_PRO3_EQ=${eq_q} ENABLE_CROSSFEED=${crossfeed_q} INSTALL_AAC_PLUGIN=${aac_plugin_q} RESTART_GADGET_NOW=${restart_gadget_q} SKIP_APT=${skip_apt_q} DISABLE_WIFI_AFTER_INSTALL=${disable_wifi_q} sudo -E bash ./install.sh"
